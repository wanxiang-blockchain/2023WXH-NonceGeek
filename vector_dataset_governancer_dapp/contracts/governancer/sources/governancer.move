module my_addr::governancer {
    ...
    public entry fun proposal(acct: &signer, title: String, content_hash: String, dataset_id: String) acquires ProposalSet {
        // Create A proposal
        ...
        emit_create_proposal_event(signer::address_of(acct), title, content_hash, dataset_id);
    }

    fun emit_create_proposal_event(voter: address, title: String, content_hash: String, dataset_id: String) acquires ProposalSet {
    ...
    public entry fun approve(acct: &signer, title: String) acquires ProposalSet, Voters {
        // Check if the acct is in Voters.
        let voters = borrow_global_mut<Voters>(@my_addr);
        let addr = signer::address_of(acct);
        assert!(vector::contains(&mut voters.voters_list, &addr), ENotValidVoter);
        // Table Change.
        let proposal_set = borrow_global_mut<ProposalSet>(@my_addr);
    ...
    public entry fun deny(acct: &signer, title: String) acquires ProposalSet, Voters {
        // Check if the acct is in Voters.
        let voters = borrow_global_mut<Voters>(@my_addr);
        let addr = signer::address_of(acct);
        assert!(vector::contains(&mut voters.voters_list, &addr), ENotValidVoter);
    ...
    #[view]
    public fun get_voters(): vector<address> acquires Voters {
        let voters = borrow_global_mut<Voters>(@my_addr);
        voters.voters_list
    }
    ...
    public entry fun reset_voter(acct: &signer) acquires ProposalSet, Voters {
        assert!(signer::address_of(acct) == @my_addr, ENotAdmin);
        // Add voter
        let voters = borrow_global_mut<Voters>(@my_addr);
